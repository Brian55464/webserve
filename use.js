...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const slider7 = document.querySelector('.slider-item:nth-child(7)');

  let isSlider7Reached = false;
  ...
  container.addEventListener('wheel', (event) => {
    ...
    // 检查是否滑动到幻灯片7
    if (isSlider7Reached === false && slider7.getBoundingClientRect().right <= window.innerWidth) {
      isSlider7Reached = true;
    }

    // 检查鼠标是否在幻灯片7的范围内
    const isMouseInSlider7 = isSlider7Reached && event.clientX >= slider7.getBoundingClientRect().left && event.clientX <= slider7.getBoundingClientRect().right && event.clientY >= slider7.getBoundingClientRect().top && event.clientY <= slider7.getBoundingClientRect().bottom;

    if (isSlider7Reached && isMouseInSlider7 && delta > 0) {
      // 在幻灯片7的范围内向下滑动，滚动到下一屏
      window.scrollTo({
      ...
      // 幻灯片内容滑动
      container.scrollLeft += scrollAmount * delta;
    }
  });
});
